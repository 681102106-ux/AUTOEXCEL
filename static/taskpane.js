...
async function filterInvoicesWithExcel(invoices) {
  ...
  await Excel.run(async (context) => {
    ...
    // 🌟 เปลี่ยนจากการล็อกเซลล์ A4 เป็นการดึงค่าจาก Named Range "CompanyTaxID"
    let myCompanyTaxId = "";
    try {
        let taxIdName = context.workbook.names.getItem("CompanyTaxID");
        let companyInfoRange = taxIdName.getRange();
        companyInfoRange.load("values");
        await context.sync();
        
        if (companyInfoRange.values && companyInfoRange.values[0] && companyInfoRange.values[0][0]) {
            let textTaxId = companyInfoRange.values[0][0].toString();
            let match = textTaxId.match(/\d{13}/);
            if (match) myCompanyTaxId = match[0];
        }
    } catch (e) {
        // ถ้าผู้ใช้ไม่ได้ตั้งชื่อ Named Range ไว้ ให้ตกไฟล์กลับมาหาเซลล์ A4 แบบเดิมเป็นตัวสำรอง
        console.log("ไม่พบ Named Range: CompanyTaxID, ถอยกลับไปใช้เซลล์ A4 แทน");
        let fallbackRange = sheet.getRange("A4");
        fallbackRange.load("values");
        await context.sync();
        if (fallbackRange.values && fallbackRange.values[0] && fallbackRange.values[0][0]) {
            let textTaxId = fallbackRange.values[0][0].toString();
            let match = textTaxId.match(/\d{13}/);
            if (match) myCompanyTaxId = match[0];
        }
    }
