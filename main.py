...
def load_vendor_master():
    global vendor_db, vendor_name_db, vendor_list_for_ui
    vendor_db.clear()
    vendor_name_db.clear()
    ...
    if os.path.exists(VENDOR_MASTER_FILE):
        with open(VENDOR_MASTER_FILE, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if clean_name:
                    vendor_name_db[clean_name] = {"v_code": v_code, "tax_id": tax_id, "name": name}
                    
                vendor_list_for_ui.append({"v_code": v_code, "tax_id": tax_id, "name": name})
        print(f"📦 โหลดฐานข้อมูลผู้ขายสำเร็จ: {len(vendor_list_for_ui)} รายการ")
    else:
        # 🌟 ถ้าไม่มีไฟล์ ให้สร้างใหม่แบบมีหัวข้อพร้อมใช้งาน
        with open(VENDOR_MASTER_FILE, "w", encoding="utf-8-sig") as f:
            f.write("v_code,tax_id,vendor_name\n")
        print("📦 สร้างไฟล์ฐานข้อมูลผู้ขายใหม่ (รอเรียนรู้จาก AI)")

# 🌟 ฟังก์ชันใหม่: ดึงเลข V-Code รันนิ่งถัดไป และบันทึกลง CSV ให้เอง
def generate_and_save_new_vcode(tax_id, vendor_name):
    global vendor_db, vendor_name_db, vendor_list_for_ui
    
    # หายอด V-Code สูงสุดในปัจจุบัน เพื่อรันเลขถัดไป
    existing_nums = []
    for v in vendor_db.values():
        code_str = v.get('v_code', '')
        if code_str.startswith('V-') and code_str[2:].isdigit():
            existing_nums.append(int(code_str[2:]))
            
    next_num = max(existing_nums) + 1 if existing_nums else 1000 # เริ่มต้นที่ V-1000
    new_vcode = f"V-{next_num}"
    
    # 🌟 เขียนเพิ่มต่อท้ายไฟล์ CSV ทันที
    with open(VENDOR_MASTER_FILE, "a", encoding="utf-8-sig", newline='') as f:
        writer = csv.writer(f)
        writer.writerow([new_vcode, tax_id, vendor_name])
        
    # อัปเดตหน่วยความจำแบบ Real-time
    vendor_db[tax_id] = {"v_code": new_vcode, "name": vendor_name}
    clean_name = vendor_name.replace(" ", "").lower()
    vendor_name_db[clean_name] = {"v_code": new_vcode, "tax_id": tax_id, "name": vendor_name}
    vendor_list_for_ui.append({"v_code": new_vcode, "tax_id": tax_id, "name": vendor_name})
    
    print(f"🧠 [AI Learning] สร้างรหัสใหม่ {new_vcode} ให้กับ {vendor_name} ({tax_id}) อัตโนมัติ!")
    return new_vcode
...
def validate_and_clean(raw_data_list, saved_image_paths):
    ...
    for data in raw_data_list:
        ai_tax_id = ''.join(filter(str.isdigit, str(data.get('tax_id', ''))))
        ai_vendor_name = str(data.get('vendor_name', '')).strip()
        clean_ai_name = ai_vendor_name.replace(" ", "").lower()
        
        data['tax_id'] = ai_tax_id
        data['v_code'] = "" 
        data['is_vendor_matched'] = False
        
        # ถ้าระบบไม่ได้ Tax ID 13 หลักมา ก็ให้ข้ามไปให้คนกรอก
        if len(ai_tax_id) != 13:
             pass 
        else:
            # 🌟 ลอจิกใหม่: ถ้าเจอเก่าให้ดึง ถ้าเป็นบริษัทใหม่ให้ "สร้างเลย!"
            if ai_tax_id in vendor_db:
                data['vendor_name'] = vendor_db[ai_tax_id]['name']
                data['v_code'] = vendor_db[ai_tax_id]['v_code']
                data['is_vendor_matched'] = True
            elif clean_ai_name in vendor_name_db:
                data['vendor_name'] = vendor_name_db[clean_ai_name]['name']
                data['v_code'] = vendor_name_db[clean_ai_name]['v_code']
                data['tax_id'] = vendor_name_db[clean_ai_name]['tax_id']
                data['is_vendor_matched'] = True
            else:
                # 🌟 ไม่เคยเจอมาก่อน! ให้ระบบสร้าง V-Code รันนิ่งใหม่ให้เลย!
                new_vcode = generate_and_save_new_vcode(ai_tax_id, ai_vendor_name)
                data['v_code'] = new_vcode
                data['is_vendor_matched'] = True
